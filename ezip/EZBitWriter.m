...
-(void) CompressAndWriteCharacter:(char) character WithCoding:(EZCodeMap*) codes {
    EZBitset* code = [codes objectForKey:@(character)];
    for (int a = 0; a < code.codelength; a++) {
        if (self.position == 16) {
            [self WriteString:self.current];
            self.position = 0;
            self.current = @"";
        }
        self.current = [self.current stringByAppendingFormat:@"%c", code.value];
        self.position++;
    }
}
...
-(void) WriteHeader:(EZTree*) tree sha:(NSString*) sha {
    ...
    for (int a = 0; a < tree.BaseNodes.count; a++) {
        header = [header stringByAppendingFormat:@"%c=%@\n", ((EZNode*)tree.BaseNodes[a]).charc, ((EZBitset*)[tree.Codes objectForKey:@(((EZNode*)tree.BaseNodes[a]).charc)]).code];
    }
